...
class Command(BaseCommand):
    ...
    def add_arguments(self, parser):
        parser.add_argument("flow_name", type=str, help="Name of the flow to execute")
        parser.add_argument(
            "--parameters",
            type=str,
            nargs="*",
    ...
    def parse_parameters(self, param_strings):
        ...
        if param_strings:
            for param in param_strings:
                if "=" not in param:
                    raise CommandError(
                        f"Invalid parameter format: {param}. Use key=value"
                    )
    ...
    def handle(self, *args, **options):
        ...
        try:
            ...
            if response.status_code != 200:
                raise CommandError(
                    f"Could not fetch flows: HTTP {response.status_code}"
                )
            ...
            payload = {"flow_id": flow_id, "parameters": parameters}

            response = requests.post(
                flow_runs_url, json=payload, timeout=options["timeout"]
            )
            ...
            if response.status_code in [200, 201]:
                ...
                if options["wait"]:
                    self.wait_for_completion(
                        prefect_api_url, run_id, options["timeout"]
                    )
            ...
            logger.error(
                "Unexpected error running flow", flow_name=flow_name, error=str(e)
            )
