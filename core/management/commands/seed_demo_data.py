...
from core.models import Organization, OrganizationMembership, OrgRole, User
...
class Command(BaseCommand):
    ...
    def clean_demo_data(self):
        ...
        demo_emails = ["admin@demo.com", "user@demo.com", "viewer@demo.com"]
    ...
    def create_admin_user(self, organization):
        ...
        membership, created = OrganizationMembership.objects.get_or_create(
            ...
            defaults={"role": OrgRole.SUPER_ADMIN, "is_default": True},
        )
    ...
    def create_regular_user(self, organization):
        ...
        membership, created = OrganizationMembership.objects.get_or_create(
            user=user,
            organization=organization,
            defaults={"role": OrgRole.EDITOR, "is_default": True},
        )

        if created:
            self.stdout.write(
                f"🔗 Created user membership with role: {membership.get_role_display()}"
            )
    ...
    def create_viewer_user(self, organization):
        ...
        membership, created = OrganizationMembership.objects.get_or_create(
            user=user,
            organization=organization,
            defaults={"role": OrgRole.VIEWER, "is_default": True},
        )

        if created:
            self.stdout.write(
                f"🔗 Created viewer membership with role: {membership.get_role_display()}"
            )
