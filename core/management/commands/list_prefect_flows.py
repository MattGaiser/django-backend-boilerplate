...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        try:
            ...
            if response.status_code == 200:
                try:
                    ...
                    if flows:
                        ...
                        logger.info("Successfully retrieved Prefect flows", 
                                  count=len(flows))
                    else:
                        self.stdout.write(
                    ...
                    self.stdout.write(
                        self.style.ERROR(f'Could not parse response from Prefect server: {str(e)}')
                    )
                    logger.error("Failed to parse flows response", error=str(e))
                    
            elif response.status_code == 404:
                self.stdout.write(
                    self.style.WARNING('Flows endpoint not found - server may not be fully configured')
                )
                ...
                logger.error("Flows request failed", 
            ...
            logger.error("Failed to connect to Prefect server for flows", 
                        url=flows_url)
            
        except requests.exceptions.Timeout:
            self.stdout.write(
                self.style.ERROR('Request to Prefect server timed out')
            )
            logger.error("Flows request timed out", timeout=options['timeout'])
            
        except Exception as e:
            self.stdout.write(
                self.style.ERROR(f'Error fetching flows: {str(e)}')
            )
        ...
        self.stdout.write('  python manage.py run_prefect_flow django-integration-example')