    ...
    def handle(self, *args, **options):
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                logger.info("Prefect server health check successful", 
                ...
                logger.warning("Prefect server health check returned non-200 status",
                             url=health_url, status_code=response.status_code)
            ...
            self.stdout.write(
                self.style.ERROR('❌ Prefect server request timed out')
            )
            logger.error("Prefect server request timed out", 
                        url=prefect_api_url, timeout=options['timeout'])
            
        except Exception as e:
            self.stdout.write(
                self.style.ERROR(f'❌ Error checking Prefect server: {str(e)}')
            )
            logger.error("Unexpected error checking Prefect server", 
                        error=str(e), url=prefect_api_url)
        ...
        self.stdout.write(f'  PREFECT_SERVER_HOST: {getattr(settings, "PREFECT_SERVER_HOST", "Not set")}')
        self.stdout.write(f'  PREFECT_SERVER_PORT: {getattr(settings, "PREFECT_SERVER_PORT", "Not set")}')