...
class CustomSocialAccountAdapter(DefaultSocialAccountAdapter):
    ...
    def pre_social_login(self, request, sociallogin):
        ...
        if not email:
            logger.warning(f"No email found in social login data for provider {sociallogin.account.provider}")
            return
    ...
    def authentication_error(self, request, provider_id, error=None, exception=None, extra_context=None):
        ...
        if error == 'access_denied':
            ...
            messages.error(request, _("Invalid request. Please contact support if this persists."))
        else:
            messages.error(request, _("Authentication failed. Please try again or contact support."))
