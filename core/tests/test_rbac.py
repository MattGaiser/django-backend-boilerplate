...
test_urlpatterns = [
    path(
        "org/<uuid:org_id>/admin-view/", RBACTestView.as_view(), name="test-rbac-view"
    ),
]
...
class TestUserHasRole(TestCase):
    ...
    def test_has_role_with_correct_role(self):
        ...
        OrganizationMembershipFactory(
            user=self.user, organization=self.org, role=OrgRole.ADMIN
        )
    ...
    def test_has_role_with_different_role(self):
        ...
        OrganizationMembershipFactory(
            user=self.user, organization=self.org, role=OrgRole.VIEWER
        )
    ...
    def test_has_role_different_users_same_org(self):
        ...
        OrganizationMembershipFactory(
            user=user2, organization=self.org, role=OrgRole.MANAGER
        )
...
class TestOrgScopedPermissionMixin(TestCase):
    ...
    def test_multiple_required_roles(self):
        ...
        OrganizationMembershipFactory(
            user=self.user, organization=self.org, role=OrgRole.MANAGER
        )
