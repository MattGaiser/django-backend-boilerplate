...
class TestOrganizationMembership(TestCase):
    ...
    def test_create_membership(self):
        ...
        membership = OrganizationMembershipFactory(
            user=self.user, organization=self.organization, role=OrgRole.ADMIN
        )
    ...
    def test_membership_unique_constraint(self):
        ...
        with self.assertRaises(IntegrityError):
            OrganizationMembership.objects.create(
                user=self.user, organization=self.organization, role=OrgRole.VIEWER
            )
    ...
    def test_multiple_organizations_for_user(self):
        ...
        membership1 = OrganizationMembership.objects.create(
            user=self.user, organization=org1, role=OrgRole.ADMIN
        )
        membership2 = OrganizationMembership.objects.create(
            user=self.user, organization=org2, role=OrgRole.VIEWER
        )
    ...
    def test_multiple_users_for_organization(self):
        ...
        membership1 = OrganizationMembership.objects.create(
            user=user1, organization=self.organization, role=OrgRole.ADMIN
        )
        membership2 = OrganizationMembership.objects.create(
            user=user2, organization=self.organization, role=OrgRole.VIEWER
        )
    ...
    def test_one_default_membership_per_user(self):
        ...
        membership1 = OrganizationMembership.objects.create(
            user=self.user, organization=org1, role=OrgRole.ADMIN, is_default=True
        )

        # Try to create second default membership - should raise ValidationError
        with self.assertRaises(ValidationError) as cm:
            membership2 = OrganizationMembership(
                user=self.user, organization=org2, role=OrgRole.VIEWER, is_default=True
            )
            membership2.save()

        self.assertIn("is_default", cm.exception.message_dict)
        self.assertIn(
            "User can only have one default organization",
    ...
    def test_can_have_multiple_non_default_memberships(self):
        ...
        membership2 = OrganizationMembership.objects.create(
            user=self.user, organization=org2, role=OrgRole.VIEWER, is_default=False
        )
        membership3 = OrganizationMembership.objects.create(
            user=self.user, organization=org3, role=OrgRole.MANAGER, is_default=False
        )

        self.assertEqual(self.user.organization_memberships.count(), 3)
        self.assertEqual(
            self.user.organization_memberships.filter(is_default=True).count(), 1
        )
        self.assertEqual(
            self.user.organization_memberships.filter(is_default=False).count(), 2
        )
...
class TestUserOrganizationMethods(TestCase):
    ...
    def test_get_membership_existing(self):
        ...
        membership = OrganizationMembership.objects.create(
            user=self.user, organization=self.org1, role=OrgRole.ADMIN
        )
    ...
    def test_get_default_organization_existing(self):
        ...
        OrganizationMembership.objects.create(
            user=self.user, organization=self.org1, role=OrgRole.ADMIN, is_default=True
        )
        OrganizationMembership.objects.create(
            user=self.user,
    ...
    def test_get_default_organization_nonexistent(self):
        ...
        OrganizationMembership.objects.create(
            user=self.user, organization=self.org1, role=OrgRole.ADMIN, is_default=False
        )
    ...
    def test_organizations_many_to_many_relationship(self):
        ...
        OrganizationMembership.objects.create(
            user=self.user, organization=self.org2, role=OrgRole.VIEWER
        )
