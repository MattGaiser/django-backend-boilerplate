...
from core.models import BaseModel, User, Organization, Project
from core.factories import UserFactory, OrganizationFactory, ProjectFactory
from core.signals import (
    set_current_user, 
    get_current_user, 
    auto_assign_user_fields,
    validate_pii_fields,
    get_pii_field_names,
...
class TestSignalBasedAuditFields(TestCase):
    ...
    def test_manual_audit_field_assignment_not_overridden(self):
        ...
        org = Organization(
            name="Test Org",
            created_by=other_user  # Manually set
        )
        org.save()
...
class TestPIIValidationSignalEnhanced(TestCase):
    ...
    def test_pii_field_names_configuration(self):
        ...
        expected_fields = {
            ...
            'phone', 'address', 'ssn', 'date_of_birth'
        }
    ...
    def test_pii_validation_with_custom_model(self):
                ...
                def get_fields(self):
                    class Field:
                        def __init__(self, name):
                            self.name = name
                    return [Field('email'), Field('personal_info'), Field('public_data')]
        ...
        try:
            ...
            if undeclared_fields:
                raise ImproperlyConfigured(f"Undeclared PII fields: {undeclared_fields}")
                
        except Exception as e:
            self.fail(f"PII validation should not raise exception for properly configured model: {e}")
            ...
            class _meta:
                ...
                def get_fields(self):
                    class Field:
                        def __init__(self, name):
                            self.name = name
                    return [Field('name'), Field('description')]  # Potentially ambiguous
        
        # The actual validation would happen in the signal
        # Here we just test the warning logic
        model_field_names = {'name', 'description'}
...
class TestSignalIntegrationWithBaseModel(TestCase):
    ...
    def test_signal_integration_with_bulk_operations(self):
        ...
        orgs_data = [
            Organization(name=f"Bulk Org {i}")
            for i in range(3)
        ]
