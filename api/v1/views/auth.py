...
def revoke_auth_token(request):
    ...
    try:
        ...
        return Response(
            {"message": _("Token revoked successfully.")}, status=status.HTTP_200_OK
        )
    except Token.DoesNotExist:
        return Response(
            {"message": _("No active token found.")}, status=status.HTTP_404_NOT_FOUND
        )
...
def auth_status(request):
    ...
    if request.user.is_authenticated:
        ...
        return Response(
            {"authenticated": True, "user": user_data}, status=status.HTTP_200_OK
        )
    else:
        return Response(
