...
from api.v1.permissions import CanViewUserData, IsOwnerOrAdmin
...
class IsAuthenticatedAndInOrgWithRoleTestCase(TestCase):
    ...
    def setUp(self):
        ...
        # Create memberships
        OrganizationMembershipFactory(
            user=self.admin_user, organization=self.organization, role=OrgRole.ADMIN
        )
        OrganizationMembershipFactory(
            user=self.manager_user, organization=self.organization, role=OrgRole.MANAGER
        )
        OrganizationMembershipFactory(
            user=self.viewer_user, organization=self.organization, role=OrgRole.VIEWER
        )
...
class ConveniencePermissionTestCase(TestCase):
    ...
    def setUp(self):
        ...
        OrganizationMembershipFactory(
            user=self.editor_user, organization=self.organization, role=OrgRole.EDITOR
        )
        OrganizationMembershipFactory(
    ...
    def test_is_org_member_permission(self):
        ...
        for user in [
            ...
            self.assertTrue(
                result,
                f"User with role {user.get_role(self.organization)} should be allowed",
            )
...
class APISpecificPermissionTestCase(TestCase):
    ...
    def setUp(self):
        ...
        OrganizationMembershipFactory(
            user=self.user1, organization=self.organization, role=OrgRole.VIEWER
        )
        OrganizationMembershipFactory(
            user=self.user2, organization=self.organization, role=OrgRole.VIEWER
        )
        OrganizationMembershipFactory(
            user=self.admin_user, organization=self.organization, role=OrgRole.ADMIN
        )
