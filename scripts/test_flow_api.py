...
try:
    ...
    print("Make sure you're running this from the project root and Django is installed.")
    sys.exit(1)
...
class PrefectFlowTester:
    ...
    def create_or_get_admin_user(self) -> tuple[User, str]:
        ...
        if membership_created:
            self.log(f"Created admin membership with role: {membership.get_role_display()}", "SUCCESS")
        elif membership.role != OrgRole.ADMIN:
            # Update role if needed
            membership.role = OrgRole.ADMIN
            membership.is_default = True
            membership.save()
            self.log(f"Updated membership role to: {membership.get_role_display()}", "INFO")
            ...
            if response.status_code == 200:
                ...
                if token:
                    ...
                    if self.verbose:
                        ...
                        self.log(f"Authenticated as: {user_info.get('full_name')} ({user_info.get('email')})", "INFO")
                    return token
                else:
    ...
    def trigger_flow(self, token: str) -> Dict[str, Any]:
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                self.log(f"Flow trigger failed with status {response.status_code}", "ERROR")
                return {
                    "success": False,
                    "status_code": response.status_code,
                    "data": error_data
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                if auth_data.get('authenticated'):
                    ...
                    self.log(f"Authenticated as: {user.get('full_name', 'Unknown')} ({user.get('email', 'Unknown')})", "SUCCESS")
                    ...
                    if orgs:
                        ...
                        for org in orgs:
                            ...
                            is_default = org.get('membership', {}).get('is_default', False)
                            default_marker = " (default)" if is_default else ""
                            self.log(f"  - {org.get('name', 'Unknown')}: {role}{default_marker}", "INFO")
                    else:
                ...
                self.log(f"Auth status check failed with status {response.status_code}", "ERROR")
    ...
    def run_test(self, create_user: bool = False, direct_flow_only: bool = False) -> bool:
        """
...
def main():
    ...
    success = tester.run_test(create_user=args.create_user, direct_flow_only=args.direct_flow_only)
